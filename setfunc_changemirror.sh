...
awk '
  # Phase 1 (first file): build a lookup table of all lines
  # NR==FNR is true while processing the first input file.
  NR == FNR {
    p[$0] = 1    # remember this exact line as a key
    n++          # count lines from the first file (not used later)
    next         # skip the rest of the rules for phase 1 lines
  }

  # Phase 2 (second file onward):
  # For each line, check whether it contains ANY key from p.
  {
    for (k in p) {
      if (index($0, k)) {  # does current line contain substring k?
        print              # if yes, print the current line and stop searching
        break              # break out of the k loop
      }
    }
  }
' /etc/apt/apt-abroad/ccDebianhttsMullvad.list /etc/apt/apt-abroad/urls.https > /etc/apt/apt-abroad/debhttpsmulmirr.list
